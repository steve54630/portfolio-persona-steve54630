...
const DEFAULT_TITLE_SIZE =
  "text-[min(9vh,3rem)] leading-none sm:text-[min(9vh,3.75rem)] md:text-[min(9vh,4.5rem)] lg:text-[min(9vh,6rem)]";

const MenuButton = forwardRef<HTMLButtonElement, MenuButtonProps>(
  ({ title, explanation, url, color, type, onClickEffect, titleSizeClassName, ...rest }, ref) => {
    const DivButton = () => (
      <button
      ...
      >
        <span
          className={`font-drunkenhour drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)] ${titleSizeClassName ?? DEFAULT_TITLE_SIZE}`}
        >
          {title}
        </span>
